        ...
        for i in range(len(state)):
            if state[i] != 0:
                if i + 1 < len(state) and state[i + 1] == 0:
                    next_states[f'D1: Disk {state[i]}'] = state[:i] + \
                        (state[i + 1],) + (state[i],) + state[i + 2:]

                if i + 2 < len(state) and state[i + 1] != 0 and state[i + 2] == 0:
                    next_states[f'D2: Disk {state[i]}'] = state[:i] + \
                        (state[i + 2],) + (state[i + 1],) + \
                        (state[i],) + state[i + 3:]

                if i - 1 >= 0 and state[i - 1] == 0:
                    next_states[f'L1: Disk {state[i]}'] = state[:i - 1] + \
                        (state[i],) + (state[i - 1],) + state[i + 1:]

                if i - 2 >= 0 and state[i - 1] != 0 and state[i - 2] == 0:
                    next_states[f'L2: Disk {state[i]}'] = state[:i - 2] + \
                        (state[i],) + (state[i - 1],) + \
                        (state[i - 2],) + state[i + 1:]
...
if __name__ == "__main__":
    discs = int(input())
    size = int(input())
    initial_state = tuple([i + 1 for i in range(discs)] + [0] * (size - discs))
    goal_state = initial_state[::-1]

    game = Discs(initial_state, goal_state)
