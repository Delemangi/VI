    ...
    def successor(self, state):
        ...
        for i, square in enumerate(state):
            for move in self.moves.keys():
                ...
                if min(new_square) < 0 or max(new_square) > 4:
                    continue

                next_states[f'Pomesti kvadratche {i + 1} {move}'] = state[:i] + (new_square,) + state[i + 1:]
...
if __name__ == '__main__':
    initial = tuple()
    goal = ((0, 0), (1, 1), (2, 2), (3, 3), (4, 4))

    for _ in range(5):
        initial += (tuple(map(int, input().split(','))), )

    squares = Squares(initial, goal)
