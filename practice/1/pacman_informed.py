...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        for direction in self.directions:
            new_state = self.generate_state(state, direction)

            x, y = new_state[0]

            if x < 0 or y < 0 or x > self.size - 1 or y > self.size - 1 or new_state[0] in self.obstacles:
                continue

            next_states[direction] = new_state
    ...
    def h(self, node):
        ...
        return min(abs(node.state[0][0] - i) + abs(node.state[0][1] - j) for i, j in node.state[2])

    def generate_state(self, state, direction):
        new_direction = (state[1] + self.directions_labels[direction]) % 4
        move = self.moves[new_direction]
        dots = list(state[2])
        coordinates = (state[0][0] + move[0], state[0][1] + move[1])

        if coordinates in dots:
            dots.remove(coordinates)

        return coordinates, new_direction, tuple(dots)
...
if __name__ == "__main__":
    ...
    initial_x = int(input())
    initial_y = 9 - int(input())
    initial_direction = directions_dict[input()]
    initial_dots_num = int(input())
    initial_dots_raw = tuple(tuple(int(i) for i in input().split(','))
                             for _ in range(initial_dots_num))
    initial_dots = tuple((i, 9 - j) for i, j in initial_dots_raw)

    initial_state = ((initial_x, initial_y), initial_direction, initial_dots)
    game = Pacman(initial_state)
